      ...
      if (endpoint_data->bInterfaceClass == USB_CLASS_HID) {
        if (endpoint_data->bInterfaceSubClass == HID_SUBCLASS_BOOT) {
          if (endpoint_data->bInterfaceProtocol == HID_ITF_PROTOCOL_KEYBOARD) {
            ...
            uint8_t b0 = transfer->data_buffer[0];
            uint8_t b1 = (transfer->actual_num_bytes > 1) ? transfer->data_buffer[1] : 0;
     
            report.buttons = ((b0 & 0xE0) == 0) ? b0 : b1;
     
            report.x     = (uint8_t)transfer->data_buffer[2];  // keep your original offsets
            report.y     = (uint8_t)transfer->data_buffer[4];  // keep your original offsets
            report.wheel = (uint8_t)transfer->data_buffer[6];  // keep your original offsets
